...
static constexpr const char * FormatModule(bool x32)
{
    // With any luck, a modern compile should just inline a routine that just returns a const string.
    if (x32)
    {
        if (IsDebugBuild())
            return ".x32.dbg.dll";
...
void TryInjectScriptablity(int pid)
{
    HANDLE process;
    BOOL x32;
    bool fail;
    ...
    std::string suffix = FormatModule(x32);

    LoadModule(process, std::string("Scriptability") + suffix);
    LoadModule(process, std::string("ScriptabilityChaiEngine") + suffix);
}